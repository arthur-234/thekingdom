...
// Removido: BackgroundGradient, pois vamos usar FocusCards
import { FocusCards, type CardItem as FocusCardItem } from "@/components/ui/focus-cards";
...
export default function MembrosPage() {
  ...
  useEffect(() => {
    ...
    (async () => {
      try {
        ...
        if (!cancelled) {
          const arr = Array.isArray(data?.members) ? (data.members as any[]) : [];
  ...
  // Construir cards para o "museu" a partir da API oficial (sem bots)
  const museumCards: FocusCardItem[] = useMemo(() => {
    const list: FocusCardItem[] = apiMembers.map((m) => ({ title: m.name, src: m.avatarUrl }));
    // Ordenar por título A-Z para efeito de "museu"
        ...
        <section>
          ...
          <div className="mt-5">
            {museumCards.length === 0 && !loadingMembers ? (
              <div className="text-neutral-400 text-sm">Ainda não há membros para exibir.</div>
            ) : (
              <FocusCards cards={museumCards} />
            )}
          </div>
        </section>
      </div>
